...
export async function addChatMessage(messageData) {
  ...
  try {
    ...
    if (newMessage.fid && newMessage.fid > 0) {
      ...
      await redis.zIncrBy(CHAT_LEADERBOARD_KEY, 1, newMessage.fid.toString());
    }
...
export async function getChatLeaderboard(limit = 10) {
  ...
  try {
    ...
    // Upstash returns: [{score: number, member: string}, ...] when WITHSCORES is true
    const topFids = await redis.zRange(CHAT_LEADERBOARD_KEY, 0, limit - 1, { rev: true, withScores: true });
    
    console.log('Leaderboard raw data type:', typeof topFids, Array.isArray(topFids));
    console.log('Leaderboard raw data:', JSON.stringify(topFids).slice(0, 500));
    ...
    // Upstash format: [{score: number, member: string}, ...]
    if (Array.isArray(topFids) && topFids[0] && typeof topFids[0] === 'object') {
      for (const item of topFids) {
        // Upstash uses 'member' for the key and 'score' for the value
        // member = FID, score = message count
        const fid = item.member || item.value;
        const messageCount = typeof item.score === 'number' ? item.score : parseInt(item.score) || 0;
        
        console.log('Processing item:', { fid, messageCount, raw: item });
        
        if (!fid) {
          console.warn('Leaderboard item missing fid:', item);
          continue;
        }
...
/**
 * Rebuild the leaderboard from actual messages (fixes corrupted data)
...
export async function rebuildLeaderboard() {
  ...
  try {
    // Get all message IDs
    const messageIds = await redis.zRange(CHAT_KEY, 0, -1);
    
    // Count messages per FID
    const counts = {};
    const userInfo = {};
    
    for (const id of messageIds) {
      const data = await redis.hGetAll(`chat:message:${id}`);
      if (data && data.fid && parseInt(data.fid) > 0) {
        const fid = data.fid;
        counts[fid] = (counts[fid] || 0) + 1;
        // Store latest user info
        userInfo[fid] = {
          user: data.user,
    ...
    // Rebuild leaderboard with correct counts
    for (const [fid, count] of Object.entries(counts)) {
      await redis.zAdd(CHAT_LEADERBOARD_KEY, { score: count, member: fid });
      // Update user info
      if (userInfo[fid]) {
        await redis.hSet(`chat:user:${fid}`, userInfo[fid]);
      }
    }
    
    console.log('Leaderboard rebuilt:', counts);
