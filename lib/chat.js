...
export async function getChatMessages(limit = 100) {
  ...
  try {
    ...
    if (messageIds.length === 0 && messageCount === 0) {
      // Redis is actually empty - show fallback messages for better UX
      console.log(`[CHAT] Redis is empty, showing fallback messages`);
      return fallbackMessages
        .slice()
        .sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp))
        .slice(-limit)
    ...
    if (messageIds.length === 0 && messageCount > 0) {
      ...
      if (allIds.length > 0) {
        ...
        messageIds.push(...allIds.slice(-limit)); // Get last N
      }
    }
    
    if (messageIds.length === 0) {
      console.log(`[CHAT] No message IDs found, returning fallback messages`);
      return fallbackMessages
        .slice()
        .sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp))
        .slice(-limit)
    ...
    console.log(`[CHAT] Successfully retrieved ${validMessages.length} valid messages out of ${messageIds.length} IDs`);
...
export async function getChatLeaderboard(limit = 10) {
  ...
  try {
    ...
    // Use zrange with rev and withscores for Upstash
    ...
    // Upstash returns array of strings: [member1, score1, member2, score2, ...]
    // OR array of objects: [{member, score}, ...]
    if (Array.isArray(topFids)) {
      // Check if it's objects or flat array
      if (topFids[0] && typeof topFids[0] === 'object' && ('member' in topFids[0] || 'score' in topFids[0])) {
        ...
        for (let i = 0; i < topFids.length; i += 2) {
          const fid = topFids[i];
          const messageCount = Number(topFids[i + 1]) || 0;
