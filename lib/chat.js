...
export async function getChatLeaderboard(limit = 10) {
  ...
  try {
    ...
    // Parse the results - Redis client may return different formats:
    // Format 1: [{value, score}, ...] (node-redis v4+)
    // Format 2: [{member, score}, ...] (Upstash Redis)
    // Format 3: [member, score, member, score, ...] (older format)
    const leaderboard = [];
    
    console.log('Leaderboard raw data:', JSON.stringify(topFids));
    
    if (Array.isArray(topFids) && topFids[0] && typeof topFids[0] === 'object') {
      // Format 1 or 2: Array of objects
      for (const item of topFids) {
        // Handle both {value, score} and {member, score} formats
        const fid = item.value || item.member;
        const messageCount = parseInt(item.score) || 0;
      ...
      // Format 3: Flat array [member, score, member, score, ...]
