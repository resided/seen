...
export async function getChatLeaderboard(limit = 10) {
  ...
  try {
    ...
    // Format 2: [member, score, member, score, ...] (older format)
    const leaderboard = [];
    
    if (Array.isArray(topFids) && topFids[0] && typeof topFids[0] === 'object' && 'value' in topFids[0]) {
      // Format 1: Array of {value, score} objects
      for (const item of topFids) {
        const fid = item.value;
