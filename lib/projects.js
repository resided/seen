...
export async function getRankedProjectsByCategory(category, limit = 10) {
  ...
  // Filter by category and status (only show queued or featured projects, excluding archived)
  const categoryProjects = projects.filter(p => 
    p.category === category && 
    (p.status === 'queued' || p.status === 'featured') &&
    p.status !== 'archived'
  );
  ...
  const projectsWithScores = await Promise.all(
    categoryProjects.map(async (project) => {
      const todayStats = await getProjectStatsToday(project.id);
...
export async function getAllCategoryRankings(limit = 10) {
  const categories = ['main', 'defi', 'social', 'games', 'tools', 'nft'];
  const rankings = {};
...
export async function submitProject(projectData) {
  const { submissionType, paymentAmount, paymentTxHash, submitterWalletAddress, ...rest } = projectData;
...
export async function createProjectDirectly(projectData, setAsFeatured = false) {
  const newProject = {
    ...
    status: setAsFeatured ? 'featured' : 'queued',
    submittedAt: new Date().toISOString(),
 ...
 */
export async function getProjectStatsToday(projectId) {
  const redis = await getRedisClient();
...
export async function updateProject(projectId, updateData) {
  ...
  // If status changed to archived and it was featured, move next queued to featured
  if (updateData.status === 'archived' && project.status === 'featured') {
    const projects = await getAllProjectsFromRedis();
    const nextQueued = projects.find(p => p.status === 'queued' && p.id !== projectId);
    if (nextQueued) {
      nextQueued.status = 'featured';
      nextQueued.featuredAt = new Date().toISOString();
      await saveProjectToRedis(nextQueued);
    }
  }
