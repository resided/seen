...
async function clearAllPersonalCooldowns(redis) {
  ...
  try {
    ...
    const allKeys = [...cooldownKeys, ...countKeys, ...donutKeys];
    if (allKeys.length > 0) {
      await redis.del(allKeys);
      console.log(`Cleared ${cooldownKeys.length} cooldowns, ${countKeys.length} claim counts, ${donutKeys.length} DONUT keys for new campaign`);
    }
    console.log('Reset DONUT count to 0 for new campaign');
  } catch (error) {
