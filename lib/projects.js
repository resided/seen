...
const ROTATION_ID_KEY = 'projects:featured:rotationId'; // Claim rotation ID - only changes on explicit reset or new featured project
...
async function saveProjectToRedis(project) {
  ...
  try {
    ...
    await redis.hSet(projectKey, {
      ...
      votes: (project.votes || 0).toString(),
...
export async function getFeaturedProject() {
  ...
  const result = featured || projects.find(p => p.status !== 'archived') || null;
  
  // CRITICAL: Attach the global rotationId to the featured project
  // This is needed for click tracking and stats to work correctly
  if (result && result.status === 'featured') {
    const rotationId = await getRotationId();
    result.rotationId = rotationId;
  }
...
/**
 * Get the current claim rotation ID for the featured project
 * This ID only changes when claims are reset or a new project becomes featured
 * Timer adjustments do NOT change this ID
 */
export async function getRotationId() {
  const redis = await getRedisClient();
  if (!redis) {
    // Fallback: use a fixed rotation ID in memory mode
    return 'fallback-rotation-1';
  }
  
  try {
    let rotationId = await redis.get(ROTATION_ID_KEY);
    
    // If no rotation ID exists, create one based on current time
    if (!rotationId) {
      rotationId = `rotation-${Date.now()}`;
      await redis.set(ROTATION_ID_KEY, rotationId);
      console.log('Created initial rotation ID:', rotationId);
    }
    
    return rotationId;
  } catch (error) {
    console.error('Error getting rotation ID:', error);
    return `fallback-rotation-${Date.now()}`;
  }
}

/**
 * Generate a new claim rotation ID
 * Call this when:
 * - A new project becomes featured (SET FEATURED)
 * - The explicit RESET CLAIMS button is pressed
 * DO NOT call this when just adjusting the timer
 */
export async function resetRotationId() {
  const redis = await getRedisClient();
  if (!redis) {
    console.warn('Cannot reset rotation ID: Redis not available');
    return null;
  }
  
  try {
    const newRotationId = `rotation-${Date.now()}`;
    await redis.set(ROTATION_ID_KEY, newRotationId);
    console.log('Reset rotation ID to:', newRotationId);
    
    // Also clear all personal cooldowns so everyone can claim on the new featured project
    await clearAllPersonalCooldowns(redis);
    
    return newRotationId;
  } catch (error) {
    console.error('Error resetting rotation ID:', error);
    return null;
  }
}

/**
 * Clear all personal claim cooldowns and DONUT eligibility
 * Called when rotation ID is reset (new featured project or manual reset)
 */
async function clearAllPersonalCooldowns(redis) {
  if (!redis) return;
  
  try {
    // Clear personal cooldown keys
    const cooldownPattern = 'claim:cooldown:*';
    const cooldownKeys = [];
    let cursor = 0;
    do {
      const [nextCursor, foundKeys] = await redis.scan(cursor, {
        MATCH: cooldownPattern,
        COUNT: 100
      });
      cursor = typeof nextCursor === 'string' ? parseInt(nextCursor, 10) : nextCursor;
      if (foundKeys && foundKeys.length > 0) {
        cooldownKeys.push(...foundKeys);
      }
    } while (cursor !== 0);
    
    // Clear personal claim count keys
    const countPattern = 'claim:count:personal:*';
    const countKeys = [];
    cursor = 0;
    do {
      const [nextCursor, foundKeys] = await redis.scan(cursor, {
    ...
    // Clear DONUT user keys (1 per campaign, not lifetime)
    const donutPattern = 'donut:user:*';
    const donutKeys = [];
    cursor = 0;
    do {
      const [nextCursor, foundKeys] = await redis.scan(cursor, {
      ...
      cursor = typeof nextCursor === 'string' ? parseInt(nextCursor, 10) : nextCursor;
      if (foundKeys && foundKeys.length > 0) {
        donutKeys.push(...foundKeys);
      }
    } while (cursor !== 0);
    
    // Clear BONUS TOKEN user keys (for admin-configured bonus tokens like DONUT)
    ...
    // Also reset the global DONUT count
    await redis.set('donut:count:given', '0');
    
    const allKeys = [...cooldownKeys, ...countKeys, ...donutKeys, ...bonusUserKeys, ...bonusCountKeys];
    if (allKeys.length > 0) {
      await redis.del(allKeys);
      console.log(`Cleared ${cooldownKeys.length} cooldowns, ${countKeys.length} claim counts, ${donutKeys.length} DONUT keys, ${bonusUserKeys.length} bonus user keys, ${bonusCountKeys.length} bonus count keys for new campaign`);
    }
    console.log('Reset DONUT count and bonus token counts to 0 for new campaign');
  } catch (error) {
    console.error('Error clearing personal cooldowns:', error);
  }
}
...
export async function getRankedProjectsByCategory(category, limit = 10) {
  ...
  const projectsWithScores = await Promise.all(
    categoryProjects.map(async (project) => {
      // For featured projects, use window stats (active accumulation)
      // For non-featured projects, use persistent stats (saved when archived)
      let displayViews, displayClicks;
      
      if (project.status === 'featured') {
        const todayStats = await getProjectStatsToday(project.id, project.rotationId);
        displayViews = todayStats.views || project.stats?.views || 0;
        displayClicks = todayStats.clicks || project.stats?.clicks || 0;
      } else {
        // Non-featured: use persistent stats (these were saved when project was archived)
        displayViews = project.stats?.views || 0;
        displayClicks = project.stats?.clicks || 0;
      }
      ...
      return {
        ...project,
        engagementScore,
        todayViews: displayViews,
        todayClicks: displayClicks,
      };
    })
  );
...
export async function getProjectById(id) {
  ...
  // CRITICAL: Attach rotationId for featured projects
  // This is needed for click tracking and stats to work correctly
  if (project && project.status === 'featured') {
    const rotationId = await getRotationId();
    project.rotationId = rotationId;
  }
...
export async function setFeaturedProject(projectId) {
  ...
  // Move current featured to active (keeps stats visible in category tabs)
  ...
  if (newFeatured) {
    ...
    // Always reset featuredAt to current time for proper auto-rotation timing
...
export async function approveAndFeatureProject(projectId) {
  ...
  // Generate new rotation ID for claims - this resets claims for everyone
  const newRotationId = await resetRotationId();
  console.log('Approve and feature project - generated new rotation ID:', newRotationId);
  ...
  for (const key of scheduledKeys) {
    ...
    if (scheduledTime <= now) {
      ...
      if (project && project.status !== 'featured') {
        ...
        // Generate new rotation ID for claims - this resets claims for everyone
        const newRotationId = await resetRotationId();
        console.log('Scheduled project now featured - generated new rotation ID:', newRotationId);
 ...
 * @param {number} projectId - Project ID
 * @param {string} rotationId - The rotation ID for featured projects (stable across timer changes)
 * @returns {Promise<{ views: number, clicks: number }>}
 */
export async function getProjectStatsToday(projectId, rotationId = null) {
  const redis = await getRedisClient();
  ...
  try {
    // For featured projects, use rotationId as the window key
    // This ensures stats persist when timer is extended, but reset with new features
    let windowKey;
    if (rotationId) {
      // Use rotation ID which only changes when a NEW project is featured
      windowKey = rotationId;
    } else {
...
export async function updateProject(projectId, updateData) {
  ...
  const updatedProject = {
    ...
    votes: updateData.votes !== undefined ? parseInt(updateData.votes) || 0 : project.votes,
    featuredAt: updateData.featuredAt !== undefined ? updateData.featuredAt : project.featuredAt,
  };

  // If status is set to featured, handle featured rotation (including re-featuring)
  if (updateData.status === 'featured') {
    const projects = await getAllProjectsFromRedis();
    // Move OTHER featured projects to active (if re-featuring same project, this will be null)
    const currentFeatured = projects.find(p => p.status === 'featured' && p.id !== projectId);
    if (currentFeatured) {
      currentFeatured.status = 'active';
      await saveProjectToRedis(currentFeatured);
    }
    // ALWAYS set featuredAt to current time (works for both new feature and re-feature)
    // This ensures everyone gets a fresh claim opportunity and timer resets
    updatedProject.featuredAt = new Date().toISOString();
    
    // Generate new rotation ID for claims - this resets claims for everyone
    const newRotationId = await resetRotationId();
    console.log('New featured project - generated new rotation ID:', newRotationId);
    
    // Clear all claim locks for the new featured rotation
    // This is done automatically by the claim system using the new featuredAt timestamp
    // Old locks will expire naturally, but we can also proactively clear them
    try {
      const redis = await getRedisClient();
      if (redis) {
        // Clear all wallet locks (they'll be recreated per new rotation)
        ...
        if (walletLockKeys.length > 0) {
          ...
          console.log(`Cleared ${walletLockKeys.length} wallet lock(s) for new featured project rotation`);
        }
        
        // Clear global wallet counters (fresh start for new featured project)
        ...
        if (globalWalletKeys.length > 0) {
          ...
          console.log(`Cleared ${globalWalletKeys.length} global wallet counter(s) for new featured project rotation`);
        }
      }
    } catch (error) {
      console.error('Error clearing claim locks for new featured project:', error);
      // Don't fail the update if lock clearing fails - locks will expire naturally
    }
  }

  // If status is changing FROM featured to something else, save the window stats to persistent stats
  if (project.status === 'featured' && updateData.status && updateData.status !== 'featured') {
    try {
      // Get current window stats before they become inaccessible
      const windowStats = await getProjectStatsToday(projectId, project.rotationId);
      console.log(`Saving window stats for project ${projectId} before status change:`, windowStats);
      
      // Merge window stats into persistent stats
      updatedProject.stats = {
        ...updatedProject.stats,
        views: windowStats.views || updatedProject.stats?.views || 0,
        clicks: windowStats.clicks || updatedProject.stats?.clicks || 0,
      };
      console.log(`Saved persistent stats for project ${projectId}:`, updatedProject.stats);
    } catch (statsError) {
      console.error('Error saving window stats to persistent:', statsError);
      // Don't fail the update if stats saving fails
    }
  }

  // When featured is archived, show SEEN. placeholder instead of auto-promoting next project
  // Admin can manually feature the next project when ready
...
/**
 * Increment vote count for a project (community voting)
 */
export async function incrementProjectVotes(projectId, voteCount = 1) {
  console.log('[INCREMENT] Called with projectId:', projectId, 'voteCount:', voteCount);
  const project = await getProjectById(projectId);

  if (!project) {
    console.log('[INCREMENT] Project not found:', projectId);
    return null;
  }

  const currentVotes = project.votes || 0;
  console.log('[INCREMENT] Current votes:', currentVotes, 'Adding:', voteCount);
  const updatedProject = {
    ...project,
    votes: currentVotes + voteCount,
  };

  console.log('[INCREMENT] Saving updated project with votes:', updatedProject.votes);
  await saveProjectToRedis(updatedProject);
  console.log('[INCREMENT] Project saved successfully');
  return updatedProject;
}

/**
 * Reset all project votes to 0 (for daily voting rounds)
 */
export async function resetAllVotes() {
  console.log('[RESET VOTES] Resetting all project votes to 0');
  const projects = await getAllProjectsFromRedis();
  ...
  for (const project of projects) {
    if (project.votes && project.votes > 0) {
      project.votes = 0;
      await saveProjectToRedis(project);
      console.log(`[RESET VOTES] Reset votes for ${project.name}`);
    }
  }

  // Also clear the Redis leaderboard sorted set
  const redis = await getRedisClient();
  if (redis) {
    await redis.del('vote:leaderboard');
    console.log('[RESET VOTES] Cleared vote leaderboard');
  }

  console.log('[RESET VOTES] All votes reset to 0');
  return { success: true, message: 'All votes reset to 0' };
}
