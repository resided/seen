...
// Each FID is cached for 60 seconds
const userCache = new Map();
const CACHE_TTL_MS = 60 * 1000; // 60 seconds

function getCachedUser(fid) {
...
function setCachedUser(fid, user) {
  userCache.set(fid, { user, timestamp: Date.now() });
  // Clean old entries periodically (keep cache from growing too large)
...
/**
 * Fetch user data from Neynar API by FID (with caching)
...
export async function fetchUserByFid(fid, apiKey) {
  // Check cache first
  const cached = getCachedUser(fid);
  if (cached !== null) {
    return cached;
  }
