...
/**
 * Fetch Farcaster miniapp rankings from Neynar
 * @param {string} apiKey - Neynar API key
 * @param {number} limit - Number of apps to fetch (default 20)
 * @returns {Promise<Object[]>} - Array of miniapp data with rankings
 */
export async function fetchMiniappRankings(apiKey, limit = 20) {
  try {
    const response = await fetch(
      `${NEYNAR_API_URL}/frame/list?limit=${limit}`,
      {
        headers: {
          'x-api-key': apiKey,
          'Accept': 'application/json',
    ...
    const data = await response.json();
    // Neynar returns frames sorted by popularity/ranking
